...
const CRITICAL_ALERTS_NOTIFY_NICO = 'input_boolean.critical_alerts_notify_person1';
const CRITICAL_ALERTS_NOTIFY_TATIANA = 'input_boolean.critical_alerts_notify_person2';
...
function showCriticalAlertsConfig() {
    ...
    const notifyPerson1 = state.getEntityState(CRITICAL_ALERTS_NOTIFY_NICO) === 'on';
    const notifyPerson2 = state.getEntityState(CRITICAL_ALERTS_NOTIFY_TATIANA) === 'on';

    criticalAlertsConfig = {
        mode: currentMode === 'off' ? 'outside' : currentMode,
        notifyPerson1: notifyPerson1,
        notifyPerson2: notifyPerson2
    };
...
function toggleCriticalNotifyPerson(person) {
    vibrate(15);
    if (person === 'person1') {
        criticalAlertsConfig.notifyPerson1 = !criticalAlertsConfig.notifyPerson1;
    } else if (person === 'person2') {
        criticalAlertsConfig.notifyPerson2 = !criticalAlertsConfig.notifyPerson2;
    }
    updateCriticalAlertsConfigUI();
}
...
function applyCriticalAlerts() {
    ...
    callService('input_boolean', criticalAlertsConfig.notifyPerson1 ? 'turn_on' : 'turn_off', {
        entity_id: CRITICAL_ALERTS_NOTIFY_NICO
    });
    callService('input_boolean', criticalAlertsConfig.notifyPerson2 ? 'turn_on' : 'turn_off', {
...
function updateCriticalAlertsConfigUI() {
    ...
    const person1Check = document.getElementById('mCriticalNotifyPerson1Check');
    const person2Check = document.getElementById('mCriticalNotifyPerson2Check');
    const person1Btn = document.getElementById('mCriticalNotifyPerson1');
    const person2Btn = document.getElementById('mCriticalNotifyPerson2');

    if (person1Check) {
        person1Check.style.display = criticalAlertsConfig.notifyPerson1 ? 'flex' : 'none';
    }
    if (person2Check) {
        person2Check.style.display = criticalAlertsConfig.notifyPerson2 ? 'flex' : 'none';
    }
    if (person1Btn) {
        person1Btn.style.borderColor = criticalAlertsConfig.notifyPerson1 ? '#10b981' : 'rgba(255,255,255,0.2)';
        person1Btn.style.borderWidth = criticalAlertsConfig.notifyPerson1 ? '3px' : '2px';
        person1Btn.style.background = criticalAlertsConfig.notifyPerson1 ? 'rgba(16, 185, 129, 0.2)' : 'rgba(16, 185, 129, 0.1)';
    }
    if (person2Btn) {
        person2Btn.style.borderColor = criticalAlertsConfig.notifyPerson2 ? '#a855f7' : 'rgba(255,255,255,0.2)';
        person2Btn.style.borderWidth = criticalAlertsConfig.notifyPerson2 ? '3px' : '2px';
        person2Btn.style.background = criticalAlertsConfig.notifyPerson2 ? 'rgba(168, 85, 247, 0.2)' : 'rgba(168, 85, 247, 0.1)';
    }
}
