...
export function showCriticalAlertsConfig() {
    ...
    const notifyPerson1 = state.getEntityState('input_boolean.critical_alerts_notify_person1') === 'on';
    const notifyPerson2 = state.getEntityState('input_boolean.critical_alerts_notify_person2') === 'on';
...
export function applyCriticalAlerts() {
    ...
    callService('input_boolean', criticalAlertsConfig.notifyPerson1 ? 'turn_on' : 'turn_off', {
        entity_id: 'input_boolean.critical_alerts_notify_person1'
    });
    callService('input_boolean', criticalAlertsConfig.notifyPerson2 ? 'turn_on' : 'turn_off', {
        entity_id: 'input_boolean.critical_alerts_notify_person2'
    });
...
function updateCriticalAlertsModalUI() {
    ...
    const person1Check = document.getElementById('criticalNotifyPerson1Check');
    const person2Check = document.getElementById('criticalNotifyPerson2Check');

    if (person1Check) {
        if (criticalAlertsConfig.notifyPerson1) {
            person1Check.classList.remove('hidden');
            person1Check.classList.add('flex');
        } else {
            person1Check.classList.add('hidden');
            person1Check.classList.remove('flex');
        }
    }

    if (person2Check) {
        if (criticalAlertsConfig.notifyPerson2) {
            person2Check.classList.remove('hidden');
            person2Check.classList.add('flex');
        } else {
            person2Check.classList.add('hidden');
            person2Check.classList.remove('flex');
        }
    }
}
